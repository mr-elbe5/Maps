L.AreaSelect = L.Class.extend({
    ...
    _setUpHandlerEvents: function(handle, xMod, yMod) {
        ...
        function onTouchMove(event) {
            dragMove(event.touches[0].pageX, event.touches[0].pageY);
        }
        function onTouchEnd(event) {
            dragEnd(event);
            L.DomEvent.off(mapContainer, "touchmove", onTouchMove);
            L.DomEvent.off(mapContainer, "touchend", onTouchEnd);
        }
        L.DomEvent.on(handle, "touchstart", (event) => {
            event.stopPropagation();
            event.preventDefault();
            dragStart(event.touches[0].pageX, event.touches[0].pageY)
            L.DomEvent.on(mapContainer, "touchmove", onTouchMove);
            L.DomEvent.on(mapContainer, "touchend", onTouchEnd);
        });
    },
